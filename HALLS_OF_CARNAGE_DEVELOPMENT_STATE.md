...
- Next reviewer slice applied: bear trap and proximity mine item displays now use upright yaw-only transforms instead of any floor-tilting rotation, the recently added modifiers were renamed/reiconed and set to weight `2`, Death Fog was also set to weight `2`, Untold Depths now supports level-type-specific normal/rare blueprint pools with global fallback, and floors 6-9 now ramp from the early exploration floor shape instead of sharing identical placeholder settings.
- Next reviewer slice applied: `/hoc recipes` is now a public player command that requires an active Halls session and opens a read-only recipe book for that session's scenario. The menu includes a building blueprint view with global and level-type pool locations, per-building detail pages with upgrade costs, and a crafting view showing Cooking Pot, Weapon Bench, and Armory recipes with their unlock levels.
- Next reviewer slice applied: recipe-book blueprint locations now render as readable per-line location lists without rarity labels or redundant blueprint naming, raw/non-vanilla Halls food such as Raw Mycelia can be eaten by right-clicking, Cinderplate damage-trigger Resistance was verified through the existing player-damage path, building menus/storage/actions now have shared audible feedback, monster files support `scale` and `movement-speed-multiplier`, Splinter children are slower/scaled through resource data, large monster spawns require a larger clear footprint, Poison Bomb was added as a cooldown utility that damages/poisons monsters only, ghosts can fly in Adventure ghost mode, and bundled building upgrade costs were increased by projected usefulness.
...
- Next reviewer slice applied: Sewer room puddles can now use room-border cells and may merge into existing liquid without renderer walls between puddles/channels. Bubble and geyser traps no longer place physical magma/soul-sand blocks in puddles; they keep both water layers and render squished bottom fixtures two blocks lower. Geysers now apply knockback and visuals without direct trap damage, and the bundled geyser damage default is `0`.
- Next reviewer slice applied: Sewer room liquid coverage was reduced to `0.45` in bundled resources and code fallback/defaults so puddles are smaller again. Geysers now temporarily turn their two water layers into bubble-column blocks during active bursts and restore water afterward. Underwater breakable drops now use a liquid-aware physics cutoff so items can settle in the puddle instead of being deleted below the normal floor plane. Zombie-to-drowned transforms are now allowed for session monsters and transfer Halls monster registration to the transformed drowned while other session monster transforms remain cancelled.

## Reviewer note (Delete entries once done, but keep the header)
Do all following for the next slice (and keep this line):
- Geyser trap still doesnt effect me if I am underwater
- Okay, a pretty big change deviating from the GDD (rewrite the part while at it):
Complete rework of camps. Instead of camps being different on all camp floors, it is going to be one big camp for the entire scenario. The camp resets if player gets game over.
Players now have (team) lives. They have 3 per scenario. If the whole team dies and gets game overed and they HAVE a life left. They loose a life and the savefile is loaded on the last camp floor. (of course the life counter gets reduced). Dying this way doesnt reset the camp. Running out of lives and getting send to floor 1 does reset the camp. If players get wiped before the first camp, they dont loose a life, since that's just a hard reset at that point.
The camp is large, you can find it under camp_untold_depths.txt. It has rooms which are separated by doors (D tile). I will get back to those later. It also has L tile, that is the point to which elevator connects.
The doors stay locked at the start, requiring a key. I will get back to keys later. If player right clicks a door, it will tell them what build plots are in the room behind the door. Right clicking with shift and having a key permanently unlocks the door.
Now about finances, instead of the normal gold UI, on camp floors there will be key counter. When arriving to camp, all extra coins party has amassed will be deposited into a "bank". By bank I mean kind of like an unlock track which gives players keys. Lets say for example first key is for 30 coins. If players arrive with 5, it will say 0 keys, 25 left. The collected coins acummulate across multiple camp visits.
The number of coins required to unlock a key are in scenario file, like a sequence, lets say first is 30, second 40, third 50. Players can unlock any reachable doors with a key. if by chance it happens a room has more doors and both sides of the door are already unlocked, the door can be opened for free.