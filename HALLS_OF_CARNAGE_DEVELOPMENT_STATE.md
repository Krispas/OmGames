...
- Floor 2+ placeholder generation now builds a deterministic four-room layout connected by basic straight corridors using those room masks.
...
- Full dungeon generation beyond the current deterministic four-room placeholder exploration floor.
...
- Elevator buttons sends the "door open" message when I press them and when the button unpresses, it triggers the transition.
- Elevator exit has visible sky, add a ceiling
- Corridor attachments at rooms have missing blocks above them, revealing the sky
- For next slice, add the actual random generation for howling_halls with the first corridor type described in the design doc.