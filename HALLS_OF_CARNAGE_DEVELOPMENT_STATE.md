...
- Overdrive spawner: Before growing large, make the hitbox smaller
- Overdrive spawner: if possible, make the spawner red when invincibility frames are on, if not possible, just give it glowing effect and make its name have red color, that should be enough
- Overdrive spawner: when shielded, make the bossbar white
- Overdrive spawner: make sounds for animations data driven too, so I can edit them on specific frames
- Make it so bosses drop scrap on death. Specifically random scrap, the amount is configred in the boss file. Overdrive spawner should drop 30. If not already applied while dropped, make sure the drops spread with some offset.
