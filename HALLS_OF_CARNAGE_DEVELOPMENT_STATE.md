...
- Make it so bear traps and proximity mines use models instead of normal physical blocks.
- Add a new level_type named backrooms, you're probably sure what it should be based around. Make a copy of the maze style corridor generation called backrooms too. This new generation will be really simillar, but on top add some long walls into the "corridors". The walls should not make it impossible to get though them.
- In debug mode, also say how exactly did the modifiers change the generation and stuff, with numbers before and after tha change.
- Add a new steam went trap which is only spawning in factory levels. Add a modifier for it which works just like modifiers for other traps, making it more common. The trap works on having intervals of safe and harmful passage when it creates smoke in 3x3 area.