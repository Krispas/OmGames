...
- Make it so monsters dont despawn by themselves
- Remove the max monster cap, instead when a monster is killed by a player, reduce the spawn cap by one, if something like slime dies and splits, add cap for the each spawned slime. Instead of raising cap limit throughout the floor, make it so the cap limit gets extended, the time between extensions is based on difficulty
- Make it so that monsters should prioritize alive players as targets
- Add a new type of item called food, food does not give hunger, but regenerates health. Add food chances to all breakables based on the chances of other stuff. Create multiple food items.
- Rework the sculk hunger effect, instead of setting hunger, just make it so player cannot eat.