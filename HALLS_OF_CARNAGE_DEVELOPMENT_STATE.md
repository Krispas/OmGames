...
- Poison bomb finally has a cooldown, good job, however there seems to be an misunderstanding in how it may work. The goal is not to apply poison to nearby enemies, but to deploy a cloud which stays in the area for some time and the cloud applies the poison.
- The library vents facing north (by facing I mean the way away from the wall they are mounted on) are great, however move them about 0.25 blocks closer to the walls. Those facing east sadly are badly placed, move them 0.25 east and 0.5 south. Those facing north have the same problem as south ones, move them 0.25 closer to the wall... wait, nevermind when searching for a west one I found the cardinal directions I covered already, but they were poistioned completely differently, why is the placement so inconsistent? Just copy the way wall spike model works?! Whats so hard about copying something we already have?
- Move the homing mine fence 0.75 blocks lower
