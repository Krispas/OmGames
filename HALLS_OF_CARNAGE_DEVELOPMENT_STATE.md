...
- Color the text in recently added scoreboard HUD. Instead of scrap taking two lines, just do one line A | B | C | D, where each letter is colored and coresponds to the collected scrap of each type. Instead of research crate, make it say only research, instand of disteller, make it say distillery. These changes should help make the UI smaller. For modifiers, show only icons with divider between them, no need for the mods text. On floors where certain features are not needed, just dont display them.
