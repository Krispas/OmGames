...
- Next reviewer slice applied: Overdrive Spawner direct melee damage now scales by Paper's vanilla attack charge curve from `Player#getCooledAttackStrength(0.5f)`, so rapid uncharged clicks with high-damage Halls weapons only deal partial damage instead of repeatedly applying full configured melee damage. Boss direct-hit handling now reports whether damage actually landed, and direct weapon poison effects/cooldown resets only happen on successful boss damage, not activation clicks or invulnerability-window clicks. Maven verification could not be run because `mvn` is not installed on PATH; targeted `javac` verification could not be run because only `javac 18` is available and the cached Paper `26.2.build.121-stable` API classes require class-file version 69.
- Next reviewer slice applied: Finished the Overdrive Spawner polish list. The boss hitbox now starts at its configured initial scale and grows with the enrage scale animation. Direct-hit invulnerability frames make the boss hitbox/displays glow, shielded minions turn the boss bar white, boss animation keyframes can define editable sound cues, and boss YAML supports `drops.random-scrap`; Overdrive Spawner now drops exactly 30 random scrap through the session-owned physics drop path so drops scatter and clean up normally. Maven verification could not be run because `mvn` is not installed and no Maven wrapper is present; targeted Java verification could not be run because only `javac 18.0.2-beta` is available while the project targets Java 25 and the cached Paper 26.2 API requires a newer class-file version.
- Next reviewer slice applied: Overdrive Spawner `drops.random-scrap` now rolls the scrap type independently for every dropped item instead of choosing one type for the whole drop batch. Boss resources support `direct-hit-invulnerability-millis` (and `direct-hit-invulnerability-seconds` as a loader alias), with Overdrive Spawner set to 1000 ms and 500 HP. Boss post-attack cooldowns now scale from the active combat floor difficulty: difficulty 20 is 0.7x, 40 is 1.0x, and 80 is 1.3x, with the 10-200 reference table and other implemented difficulty effects documented in `Extra Resources/halls_of_carnage/difficulty.md`. Boss floor spawns now center on the open-cell bounds of the boss layout, so larger or bordered arenas keep the boss centered in the playable room.
- Next reviewer slice applied: Halls player save state now persists current health additively under each saved player entry, with old saves falling back to full health. New sessions and lobby returns restore players to full current max health, while loaded saves and camp-checkpoint restores restore the saved health after saved totem modifiers are reapplied. Lobby preparation only full-heals non-session players so reconnecting active participants are not healed accidentally. Overdrive Spawner now tracks when summoning has been available for at least 5 seconds and gives the summon attack triple selection weight while it remains available.

## Reviewer note (Delete entries once done, but keep the header)
Do all following for the next slice (and keep this line):
- Boss 2 - Add second boss: Archaic Guard, documented in archaic_guard.md. Use boss_floor.md for design principles (and existing boss design too, all mechanics between bosses should behave consistently). Place it on the last fight floor. While working on it, merge behaviour of poison bomb poioson cloud, librarian poison cloud and the poison clouds from the boss, of course the poison bomb one effects monsters only and the other two ones players only.
