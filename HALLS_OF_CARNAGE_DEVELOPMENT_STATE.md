...
- Make it so blueprint distellery has a model which is editable through resources, like buildings
- Okay more about the vent iron bars in library generation, since youre still getting it wrong, the bars should be IN the entrance of 1 wide corridors. If 1 wide corridor intersects 3 wide corridor, just put the bars IN the entrances into the 1 wide, not THROUGH the 3 wide corridor. Also dont forget to put the bars on 1 wide entrances in rooms.
- The bunker generation is now fully broken, revert to what you had before. Right now only 1 room with connecting corridor generates, the corridor also generates in a way that its often block. I want a long corridor wrapping through around most rooms around the level with small corridors leading into rooms. Also the redstone lamps in main corridor are off, but lets make it so 15% of them are on.
