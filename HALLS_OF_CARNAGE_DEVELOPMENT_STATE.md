...
- The poison bomb is still not spawning a poison cloud, if there is a problem with the detection, just spawn it on players position, but I am not sure if thats where the problem lies.
- Amazing, you finally almost nailed the library vent models, one last chance, dont screw this up, the vents are not touching walls right now, move the whole model 0.25 towards the wall its sitting on.
