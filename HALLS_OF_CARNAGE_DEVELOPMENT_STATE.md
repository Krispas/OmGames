...
- The iron bars in library generation are still not connecting to walls properly, the intersection of small and large corridors is still blocked by them that shouldnt happen, it should work like when they generate in rooms!
- Make it so the chance for lamps being on in the large bunker corridor is 40% instead of 15%
- For some reason, holes dont generate anymore, they used to generate just fine, what happened? find out and fix it.
- Crafting window in /hoc recipes is missing arrows and not everything can fit on one page, make sure it also works for normal crafting windows
