...
- Instead of "Gather what you can" when entering exploration floor, display the name of the level type, add the field to the level types and also add a field which tells the color of the name.
- The sewer holes are quite small, is it possible to make it so holes generate first and water second to give them more space?
- Make it so bear traps can also be disarmred by throwing items on them
- Make it so throwing items on proximity mines and bear trap doesnt delete them
- Make poison bomb some other item, lets say firework star while having model worthy of poison bomb, give it the classic cooldown other utilities have and particles
- Give enchanted book trap 10hp before it breaks
- Make particles after rotting soldier explosion take the space of the whole explosion (if it already does that, incraese the range)
- Monsters spawned by coffin traps seem to have their attributes not applied (brooding mother was normal size)
- Make it so monsters like brooding mother and splinter family splitting is data driven, for example if I wanted I should be able to make any other mob behave like them
- Homing mine model is still incorect, make it rotated the same way proximity mines are rotated, not standing on the side.
- Library vents facing west are almost okay, just put them 0.2 blocks closer to the wall, the ones facing south are completely wrong, dont forget block displayes dont rotate at their center
- Make more vents spawn in the library
- Items dropped onto the buildings teleport out of the hitbox now, perfect, but add the same behaviour to empty build slots.
