...
- In the recently colored HUD, wood scrap should be brown, if that is not an available color, use GOLD. Instead of research open/done, display x and checkmark, colored.
- Make it so research crates cannot spawn in the room right before the elevator
- In library level type, make it so vents are blocked off with iron bars, when right clicked, player will either enter or exit the vent, this makes it impossible for monsters to follow in.
- The bunker corridor generation doesnt work as it should. As I said, there is a large main corridor which wraps throughout the level, from which simple 1 block wide tunnels lead into rooms. If two rooms are connected by extra corridor, it is always the small one.
- Add a new feature, blueprint distillery. The distilery has a model. When going to exploration floor, 5 points on the floor are picked and reserved for the distillery. The distilleries generate there. When interacted with, they become enabled. When enabled they will create a particle beam leading towards the next inactive distillery (a direction, not route, for example it can point into a wall). Once all 5 are active, the player who activated the last one receives a blueprint based on level type pool. With 33% of it being a rare blueprint.
...
- Make speed totem, grindstone and deconstructor into normal rarity buildings.
...
- Rework blueprint distribution, there are now no bluprint types shared amongst all level types, rework it based on Extra Resources/halls_of_carnage/UntoldDepthsBuildingPools.csv
