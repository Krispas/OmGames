...
- Make it so the geyser trap also affect vertically the liquid blocks
- Breakables which are underwater still seem to not spawn items when broken.
- If zombie converts to drowned, dont stop registering it as an enemy.
- Puddles are way too large now, make them smaller