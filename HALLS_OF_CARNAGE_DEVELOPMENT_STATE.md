...
- Next reviewer slice applied: The level-type-specific hole failure was traced to `floorReachableWithout` using a hardcoded south elevator start cell while Halls generated exploration floors use the north elevator front. This only broke level types with global reachability checks enabled: Howling Corridors, Frozen Halls, Infernal Chambers, Sewer, Library, and Bunker. The reachability check now resolves the actual generated elevator-adjacent walkable start cell from all four sides, preserving global reachability validation without rejecting valid holes for north-front floors. Maven verification could not be run in this environment because neither `mvn` nor a Maven wrapper is available.
- Next reviewer slice applied: Wall spike line damage now ignores players and monsters below the generated floor plane, so underwater/lower-floor entities are not hit by wall spikes. Proximity mines can now be disarmed by throwing a Halls physics drop near them. Hole placement rejects pit masks with adjacent liquid cells, preserving at least a one-block ground border around holes. Poison Bomb now uses a non-throwable `SPIDER_EYE` base item with the model hook retained so custom utility handling owns activation instead of vanilla splash-potion throwing. Rotting Soldier death now shows a temporary TNT block-display fuse with beeps before a larger non-block-breaking explosion. Monster resources now support optional `attack-damage`, and Spiderling is tuned to 1 damage. Enchanted Book traps and Army Coffins now spawn interaction hitboxes; enchanted books can be permanently destroyed by attacking the hitbox, and coffins can be opened/broken through it. Homing Mine now uses the same pressure-plate style as the normal mine but with a gold pressure plate, assigns its moving display as the active mine location, chases nearby players, and detonates when it catches them. Maven verification could not be run in this environment because neither `mvn` nor a Maven wrapper is available.
- Next reviewer slice applied: Library vent display placement now uses the same wall-face anchor/depth convention as wall trap fixtures, and the bundled `library_vent.yml` offsets were converted to positive wall depths so vent displays sit on the selected wall face instead of rotating around the floor-cell center. Sculk Purifier blueprint was corrected from rare to normal to match the Untold Depths building blueprint pools. Halls physics drops now ask camp runtime to eject them from built camp plot hitboxes, pushing drops just outside the building footprint so they remain reachable. Maven verification could not be run in this environment because neither `mvn` nor a Maven wrapper is available.
- Next reviewer slice applied: Untold Depths floor definitions were rebuilt from `Extra Resources/halls_of_carnage/UntoldDepthsLevels.csv`. The scenario now has explicit floors through 52, camps at the CSV positions, floor 50 as a normal Howling Corridors exploration floor, and temporary `combat` placeholders using `special/final_floor_1.txt` for the Overdrive Spawner and Archaic Guard rows until their dedicated boss slices are implemented.
...
- Bosses (overdrive spanwer) can currently be spawned clicked at full damage, try to find some other ways, I know datapack creators use invisible pigs for damage detection, but I dont know whats the modern approach (maybe search online + you have plugin, no datapack restrictions)
- When boss dies, every other monster in the room should die too (and in case of splitting monsters, they shouldnt spawn children)
- Make the overdrive spawner not use custom model, just use a giant spawner, dont forget to give it particles and stuff.
- The shockwaves overdrive spawner generates are too fast and make jumping above them hard.
