...
- Overdrive spawner on floor 26 doesnt use the level file for some reason... why?
- Ovedrive spawner - make the whole xblast attack about 33% faster, so player needs to react more quickly. Also if not already, make the rotation of the blast completely random.
- Place a lamp into the floor right under the boss on boss floors. Also generate symetrical pattern of lights throughout the boss arena so its more lit.
