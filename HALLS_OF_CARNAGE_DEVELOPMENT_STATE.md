...
- Right now, monsters approach me from far away even if I am doing totally nothing, thats kind of an issue
- Make the rising monster cap faster, on difficulty 10, lets say 60 seconds, on difficulty 80, lets say 20 seconds
- - Implement the modifier system as writting in GDD
- When spawning monsters, choose only the basic monsters for the level type. If the special enemy modifier is picked, select one enemy fromt he specials and add it to the spawning pool with a little bit lesser weigth
- If a trap is rolled as a modifier (for example the poison or falling ice one), double the chance of picking that trap type on the floor and also spawn 33% more traps (multiplicative if other buffs are chosen)
- When riding elevator to an exploration floor, pick 3 modifiers and display them on the HUD as a slot machine. Good modifiers are green, bad are red. The chances of good modifier for each slot are (50 - difficulty)%. Multipliers can repeat and if they do, their effect stacks or multiplies based on the type. Add sound effects for this. Each modifier should have icon which is  asingle emoji like ☺ ☻ ♥ ♣, minecraft supports unicode, so support it too, these icons can be configured in resoruces
- Add a good modifier called Compass. Having it once gives player a compass pointing to the elevator, having it twice also changes HUD, telling player in blocks exactly how far they are, having it thrice will make a trail of particles towards the elevator from each player each 5 seconds.