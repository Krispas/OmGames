...
- Halls boss archetypes currently define `id`, `name`, `health`, `multiplayer-hp-boost`, display material/model/parts, optional display animations, optional animation keyframe sounds (`sound`/`volume`/`pitch` or `sounds` entries), `drops.random-scrap`, and boss-specific sections such as `overdrive` attack timing, damage, spawn count, max live minions, minion respawn cooldown, phase scale multipliers, shockwave chain ranges, enraged X-blast chains, and weighted spawn pool. Boss runtime is session-owned through `HallsSessionBossRuntime`, not normal monster AI. Overdrive Spawner minions shield the boss while alive; below half health, spawn waves use the full configured minion cap and start their doubled cooldown only after the full wave is cleared.
