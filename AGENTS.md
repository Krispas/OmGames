...
- Terminal editing gives the account owner/editor or Bank admin an enchanted stick editor item tagged with the terminal id.
- Left-clicking with a terminal editor opens that terminal's sellable item list; dropping the editor item despawns it, and End Edit removes editor items from the player inventory.
- Terminal sellable items store display name, price, icon material, sort order, and an optional exact block click location.
- Setting a terminal item click location is a two-step editor flow: choose Set Click Block in the item menu, then right-click the target block with the editor stick.
- Right-clicking a registered terminal item click block with a valid Bank credit card in either hand adds that sellable item to the player's cart.
...
- Terminal checkout currently transfers credits only; physical delivery/inventory integration for purchased goods is intentionally pending.
...
- Breakable display parts support decimal `offset`, optional `block-data`, and `rotation`/`euler` `[x, y, z]` degrees.
...
- Building files define `id`, `name`, `size` (`small`, `medium`, `large`, or the permanent `station` camp-station size), `blueprint`, `implemented`, and `levels.<1|2|3>` with display `parts`, optional `empty-parts`, optional `upgrade-cost` stored-scrap requirements, optional `interaction.give-items` compatibility outputs, and optional `harvest.uses` / `harvest.items` for harvestable buildings.
- Camp Station uses the reserved `camp_station` building id on 7x7 camp plot components, is always present, has no blueprint, cannot be upgraded or destroyed, and owns the scenario's combined Cooking Pot, Weapon Bench, and Armory recipes.
...
- Item recipes are parsed as stored-scrap and hotbar item costs. Camp Station lists scenario-unlocked recipes and crafts them from its building GUI.
...
- Halls boss archetypes currently define `id`, `name`, optional `ai`, `health`, `multiplayer-hp-boost`, `direct-hit-invulnerability-millis`, display material/model/parts, optional display animations, optional animation keyframe sounds (`sound`/`volume`/`pitch` or `sounds` entries), `drops.random-scrap`, and boss-specific sections such as `overdrive` and `archaic-guard`. Boss runtime is session-owned through `HallsSessionBossRuntime`, not normal monster AI. Overdrive Spawner minions shield the boss while alive; below half health, spawn waves use the full configured minion cap and start their doubled cooldown only after the full wave is cleared. Archaic Guard uses missile, shockwave, radial-wall, spawn, and reposition attacks, enrages at its configured phase threshold, and drops player-targeting poison/wither clouds through the shared Halls poison-cloud helper. Boss post-attack cooldowns scale by floor difficulty; the reference table is `Extra Resources/halls_of_carnage/difficulty.md`.
