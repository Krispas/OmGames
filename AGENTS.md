...
- Halls boss archetypes currently define `id`, `name`, `health`, `multiplayer-hp-boost`, display material/model/parts, and boss-specific sections such as `overdrive` attack timing, damage, spawn count, and weighted spawn pool. Boss runtime is session-owned through `HallsSessionBossRuntime`, not normal monster AI.
